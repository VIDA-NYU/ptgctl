...
#         i = round((t.inputBufferAdcTime - self.t0) * self.sr)
...
#         # samples since first output buffer
#         i = round((t.outputBufferDacTime - self.t0) * self.sr)

#         # get the output buffer positions
#         ibuf = self.start_pos + i
#         jbuf = ibuf + frames

#         # allow fast-forward/rewind to skip blanks between recordings
#         pos = q.queue[0][0]
#         print('pos, buf', pos, [ibuf, jbuf])
#         if jbuf < pos or self.start_pos > pos or self.last_pos > pos:  # pos is outside bounds
#             print('ff/rw', pos, pos - ibuf)
#             # shift offset so that ibuf lines up with pos
#             self.start_pos += pos - ibuf
#             ibuf = pos
#             jbuf = ibuf + frames
#         self.last_pos = pos
...
#             if right > left:
#                 buf[left - ibuf:right - ibuf] = y[left - pos:right - pos]
#             print(pos, ibuf, left, right)
